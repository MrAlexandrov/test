...
int32_t main() {
    ...
    vector <PT> conv = convex_hull(a);
    int E = 3 * n - 3 - (int)conv.size();

    vector <pair<line, pair<int, int>>> good; /// {line, {ind, ind}}    /// unused segment
    vector <line> bad;                              /// used segment
    vector <bool> ok(n * n, true);

    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            good.push_back({line(a[i], a[j]), {i + 1, j + 1}});
        }
    }
    ...
    if (E & 1) {
        --E;
        cout << 1 << '\n';
        cout << good.back().second.first << ' ' << good.back().second.second << endl;
        ok.back() = false;
        bad.push_back(good.back().first);
    } else {
        cout << 2 << endl;
    }
    for (int i = 0; i < E / 2 + 1; ++i) {
        ...
        bob1 = a[f - 1], bob2 = a[s - 1];
        for (int j = 0; j < (int)good.size(); ++j) {
            if (ok[j] && good[j].first.a == bob1 && good[j].first.b == bob2 /* || good[j].a == bob2 && good[j].b == bob1*/) {
                ok[j] = false;
                bad.push_back(line(bob1, bob2));
                break;
            }
        }
        for (int j = 0; j < (int)good.size(); ++j) {
            if (ok[j]) {
                bool now = true;
                for (int k = 0; k < (int)bad.size() && now; ++k) {
                    PT intersect;
                    if (seg_seg_intersection(good[j].first.a, good[j].first.b, bad[k].a, bad[k].b, intersect)) {
                        now = false;
                    }
                }
                if (now) {
                    ok[j] = false;
                    bad.push_back(good[j].first);
                    cout << good[j].second.first << ' ' << good[j].second.second << endl;
                    break;
                }
            }
        }
